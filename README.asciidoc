...
* Install +virtualbox+, we assume you know how to do that on your OS, if you
  are using openSUSE/SLED you need to grab the latest Virtualbox version from
  the https://build.opensuse.org/project/show/Virtualization[Virtualization]
  project to be able to compile the guest additions on the SLES 12 virtual machine.
...
* Install the +vbguest+ plugin for vagrant with the following command:
